...
new google.maps.LatLng('0','0');var marker=new google.maps.Marker({position:latLng,map:map,icon:imageMarkerList,animation:google.maps.Animation.DROP,});markers.push(marker);bounds.extend(marker.position);if(elementSettings.enable_infoWindow){google.maps.event.addListener(marker,'click',(function(marker,k){return function(){if(elementSettings.infoWindow_click_to_post){window.location=positions[k].link}else{var iwOptions={content:positions[k].infowindow,}
...
infoWindowMap.open(map,marker);lastWindow=infoWindowMap}}})(marker,i))}}
...
function infoWindow(marker){if(elementSettings.enable_infoWindow){if(elementSettings.infoWindow_click_to_url&&elementSettings.infoWindow_url){marker.addListener('click',function(){window.location=elementSettings.infoWindow_url.url})}else if(positions[0].infowindow){var iwOptions={content:positions[0].infowindow,}
if(elementSettings.infoWindow_panel_maxwidth.size){iwOptions.maxWidth=elementSettings.infoWindow_panel_maxwidth.size}
